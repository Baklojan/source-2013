...
#include "ammodef.h"
	...
	#include "team.h"
	#include "voice_gamemgr.h"
	#include "mod_gameinterface.h"
	#include "mod_cvars.h"

#ifdef DEBUG	
	#include "mod_bot_temp.h"
#endif
...
REGISTER_GAMERULES_CLASS( CModRules );

BEGIN_NETWORK_TABLE_NOBASE( CModRules, DT_ModRules )
...
LINK_ENTITY_TO_CLASS( mod_gamerules, CModGameRulesProxy );
IMPLEMENT_NETWORKCLASS_ALIASED( ModGameRulesProxy, DT_ModGameRulesProxy )

static ModViewVectors g_ModViewVectors(
...
static const char *s_PreserveEnts[] =
{
...
#ifdef CLIENT_DLL
	void RecvProxy_ModRules( const RecvProp *pProp, void **pOut, void *pData, int objectID )
	{
		CModRules *pRules = ModRules();
		Assert( pRules );
		*pOut = pRules;
	}

	BEGIN_RECV_TABLE( CModGameRulesProxy, DT_ModGameRulesProxy )
		RecvPropDataTable( "mod_gamerules_data", 0, 0, &REFERENCE_RECV_TABLE( DT_ModRules ), RecvProxy_ModRules )
	END_RECV_TABLE()
#else
	void* SendProxy_ModRules( const SendProp *pProp, const void *pStructBase, const void *pData, CSendProxyRecipients *pRecipients, int objectID )
	{
		CModRules *pRules = ModRules();
		Assert( pRules );
		return pRules;
	...
	BEGIN_SEND_TABLE( CModGameRulesProxy, DT_ModGameRulesProxy )
		SendPropDataTable( "mod_gamerules_data", 0, &REFERENCE_SEND_TABLE( DT_ModRules ), SendProxy_ModRules )
	END_SEND_TABLE()
#endif
...
const CViewVectors* CModRules::GetViewVectors()const
{
	return &g_ModViewVectors;
}

const ModViewVectors* CModRules::GetModViewVectors()const
{
	return &g_ModViewVectors;
}
	
CModRules::~CModRules( void )
{
#ifndef CLIENT_DLL
...
void CModRules::CreateStandardEntities( void )
{
	...
	CBaseEntity *pEnt = 
#endif
	CBaseEntity::Create( "mod_gamerules", vec3_origin, vec3_angle );
	Assert( pEnt );
#endif
}
...
//=========================================================
float CModRules::FlWeaponRespawnTime( CBaseCombatWeapon *pWeapon )
{
#ifndef CLIENT_DLL
...
bool CModRules::IsIntermission( void )
{
#ifndef CLIENT_DLL
...
void CModRules::PlayerKilled( CBasePlayer *pVictim, const CTakeDamageInfo &info )
{
#ifndef CLIENT_DLL
	if ( IsIntermission() )
		return;
...
void CModRules::GoToIntermission( void )
{
#ifndef CLIENT_DLL
	if ( g_fGameOver )
		return;
...
// to spawn again.
//=========================================================
float CModRules::FlWeaponTryRespawn( CBaseCombatWeapon *pWeapon )
{
#ifndef CLIENT_DLL
...
//=========================================================
Vector CModRules::VecWeaponRespawnSpot( CBaseCombatWeapon *pWeapon )
{
#ifndef CLIENT_DLL
	CWeaponModBase *pHL2Weapon = dynamic_cast< CWeaponModBase*>( pWeapon );
...
CWeaponModBase* IsManagedObjectAWeapon( CBaseEntity *pObject )
{
	return dynamic_cast< CWeaponModBase*>( pObject );
}
...
bool GetObjectsOriginalParameters( CBaseEntity *pObject, Vector &vOriginalOrigin, QAngle &vOriginalAngles )
{
	...
	else if ( CWeaponModBase *pWeapon = IsManagedObjectAWeapon( pObject )) 
	{
...
void CModRules::ManageObjectRelocation( void )
{
...
//=========================================================
void CModRules::AddLevelDesignerPlacedObject( CBaseEntity *pEntity )
{
...
//=========================================================
void CModRules::RemoveLevelDesignerPlacedObject( CBaseEntity *pEntity )
{
	if ( m_hRespawnableItemsAndWeapons.Find( pEntity ) != -1 )
	{
...
// Some game variations may choose to randomize spawn locations
//=========================================================
Vector CModRules::VecItemRespawnSpot( CItem *pItem )
{
...
//=========================================================
QAngle CModRules::VecItemRespawnAngles( CItem *pItem )
{
...
//=========================================================
float CModRules::FlItemRespawnTime( CItem *pItem )
{
...
//=========================================================
bool CModRules::CanHavePlayerItem( CBasePlayer *pPlayer, CBaseCombatWeapon *pItem )
{
	if ( weaponstay.GetInt() > 0 )
	{
...
//=========================================================
int CModRules::WeaponShouldRespawn( CBaseCombatWeapon *pWeapon )
{
#ifndef CLIENT_DLL
...
//-----------------------------------------------------------------------------
void CModRules::ClientDisconnected( edict_t *pClient )
{
#ifndef CLIENT_DLL
...
// Deathnotice. 
//=========================================================
void CModRules::DeathNotice( CBasePlayer *pVictim, const CTakeDamageInfo &info )
{
#ifndef CLIENT_DLL
...
void CModRules::ClientSettingsChanged( CBasePlayer *pPlayer )
{
#ifndef CLIENT_DLL
	
	CMod_Player *pHL2Player = ToModPlayer( pPlayer );
...
int CModRules::PlayerRelationship( CBaseEntity *pPlayer, CBaseEntity *pTarget )
{
#ifndef CLIENT_DLL
...
const char *CModRules::GetGameDescription( void )
{ 
	if ( IsTeamplay() )
...
bool CModRules::IsConnectedUserInfoChangeAllowed( CBasePlayer *pPlayer )
{
	return true;
}
 
float CModRules::GetMapRemainingTime()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CModRules::Precache( void )
{
	CBaseEntity::PrecacheScriptSound( "AlyxEmp.Charge" );
}

bool CModRules::ShouldCollide( int collisionGroup0, int collisionGroup1 )
{
...
bool CModRules::ClientCommand( CBaseEntity *pEdict, const CCommand &args )
{
#ifndef CLIENT_DLL
	...
	CMod_Player *pPlayer = (CMod_Player *) pEdict;
...
CAmmoDef *GetAmmoDef()
{
	...
	bool CModRules::FShouldSwitchWeapon( CBasePlayer *pPlayer, CBaseCombatWeapon *pWeapon )
	{		
...
void CModRules::RestartGame()
{
	...
	for (int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		CMod_Player *pPlayer = (CMod_Player*) UTIL_PlayerByIndex( i );
...
void CModRules::CleanUpMap()
{
	...
	while ( pCur )
	{
		CBaseModCombatWeapon *pWeapon = dynamic_cast< CBaseModCombatWeapon* >( pCur );
	...
	class CModMapEntityFilter : public IMapEntityFilter
	{
	public:
...
void CModRules::CheckChatForReadySignal( CMod_Player *pPlayer, const char *chatmsg )
{
...
void CModRules::CheckRestartGame( void )
{
...
void CModRules::CheckAllPlayersReady( void )
{
	for (int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		CMod_Player *pPlayer = (CMod_Player*) UTIL_PlayerByIndex( i );
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *CModRules::GetChatFormat( bool bTeamOnly, CBasePlayer *pPlayer )
{
