...
void Bot_Think( CMod_Player *pBot );
...
CBasePlayer *BotPutInServer( bool bFrozen, int iTeam )
{
	...
	CMod_Player *pPlayer = ((CMod_Player *)CBaseEntity::Instance( pEdict ));
	pPlayer->ClearFlags();
...
void Bot_RunAll( void )
{
	...
	{
		CMod_Player *pPlayer = ToModPlayer( UTIL_PlayerByIndex( i ) );
...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove( CMod_Player *fakeclient, const QAngle& viewangles, float forwardmove, float sidemove, float upmove, unsigned short buttons, byte impulse, float frametime )
{
	if ( !fakeclient )
		return;
