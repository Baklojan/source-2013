...
CModPlayerAnimState* CreateModPlayerAnimState( CMod_Player *pPlayer )
{
	...
	CModPlayerAnimState *pRet = new CModPlayerAnimState( pPlayer, movementData );

	// Specific HL2MP player initialization.
	pRet->InitModAnimState( pPlayer );
...
// Input  :  - 
//-----------------------------------------------------------------------------
CModPlayerAnimState::CModPlayerAnimState()
{
	m_pModPlayer = NULL;

	// Don't initialize HL2MP specific variables here. Init them in InitModAnimState()
}
...
//			&movementData - 
//-----------------------------------------------------------------------------
CModPlayerAnimState::CModPlayerAnimState( CBasePlayer *pPlayer, MultiPlayerMovementData_t &movementData )
...
// Input  : *pPlayer - 
//-----------------------------------------------------------------------------
void CModPlayerAnimState::InitModAnimState( CMod_Player *pPlayer )
{
	m_pModPlayer = pPlayer;
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CModPlayerAnimState::ClearAnimationState( void )
{
...
// Output : Activity
//-----------------------------------------------------------------------------
Activity CModPlayerAnimState::TranslateActivity( Activity actDesired )
{
	...
	if ( GetModPlayer()->GetActiveWeapon() )
	{
		translateActivity = GetModPlayer()->GetActiveWeapon()->ActivityOverride( translateActivity, false );
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CModPlayerAnimState::Update( float eyeYaw, float eyePitch )
{
	// Profile the animation update.
	VPROF( "CModPlayerAnimState::Update" );

	// Get the HL2MP player.
	CMod_Player *pModPlayer = GetModPlayer();
	if ( !pModPlayer )
		return;

	// Get the studio header for the player.
	CStudioHdr *pStudioHdr = pModPlayer->GetModelPtr();
	if ( !pStudioHdr )
		return;
	...
	{
		m_pModPlayer->SetPlaybackRate( 1.0f );
	}
#endif
}
...
// Input  : event - 
//-----------------------------------------------------------------------------
void CModPlayerAnimState::DoAnimationEvent( PlayerAnimEvent_t event, int nData )
{
	...
	case PLAYERANIMEVENT_ATTACK_PRIMARY:
		{
			// Weapon primary fire.
			if ( m_pModPlayer->GetFlags() & FL_DUCKING )
	...
	{
		CBaseCombatWeapon *pWeapon = GetModPlayer()->GetActiveWeapon();
		if ( pWeapon )
		{
...
// Input  : *idealActivity - 
//-----------------------------------------------------------------------------
bool CModPlayerAnimState::HandleSwimming( Activity &idealActivity )
{
...
//-----------------------------------------------------------------------------
bool CModPlayerAnimState::HandleMoving( Activity &idealActivity )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CModPlayerAnimState::HandleDucking( Activity &idealActivity )
{
	if ( m_pModPlayer->GetFlags() & FL_DUCKING )
	{
...
//-----------------------------------------------------------------------------
// Purpose: 
bool CModPlayerAnimState::HandleJumping( Activity &idealActivity )
{
	Vector vecVelocity;
	...
	if ( m_bJumping )
	{
		...
		if ( m_pModPlayer->GetWaterLevel() >= WL_Waist )
		{
			m_bJumping = false;
			RestartMainSequence();
		}
		...
		{
			if ( m_pModPlayer->GetFlags() & FL_ONGROUND )
			{
				m_bJumping = false;
				RestartMainSequence();
...
bool CModPlayerAnimState::SetupPoseParameters( CStudioHdr *pStudioHdr )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CModPlayerAnimState::EstimateYaw( void )
{
	// Get the frame time.
...
//-----------------------------------------------------------------------------
float SnapYawTo( float flValue );
void CModPlayerAnimState::ComputePoseParam_MoveYaw( CStudioHdr *pStudioHdr )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CModPlayerAnimState::ComputePoseParam_AimPitch( CStudioHdr *pStudioHdr )
{
	// Get the view pitch.
...
// Purpose: 
//-----------------------------------------------------------------------------
void CModPlayerAnimState::ComputePoseParam_AimYaw( CStudioHdr *pStudioHdr )
{
...
// Output : float
//-----------------------------------------------------------------------------
float CModPlayerAnimState::GetCurrentMaxGroundSpeed()
{
